    ...
    >
      <el-row>
        ...
        <el-form-item class="select" v-if="type!='info'" label="职位" prop="zhiwei">
          <el-select  @change="zhiweiChange" v-model="ruleForm.zhiwei" placeholder="请选择职位">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.zhiwei" label="职位" prop="zhiwei">
          ...
          <el-form-item class="input" v-if="ruleForm.jieshushijian" label="结束时间" prop="jieshushijian">
              <el-input v-model="ruleForm.jieshushijian" 
                placeholder="结束时间" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="当前进度" prop="dangqianjindu">
          <el-select v-model="ruleForm.dangqianjindu" placeholder="请选择当前进度">
            <el-option
                v-for="(item,index) in dangqianjinduOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="当前进度" prop="dangqianjindu">
	      <el-input v-model="ruleForm.dangqianjindu"
                placeholder="当前进度" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                              </el-row>
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jieshushijian = true;
            continue;
	            	            ...
	            	            if(o=='dangqianjindu'){
            this.ruleForm.dangqianjindu = obj[o];
	    this.ro.dangqianjindu = true;
            continue;
          }
	                    }
                                                                                                              }
          ...
          this.$message.error(data.msg);
              ...
              if (data && data.code === 0) {
                ...
                this.$message.error(data.msg);
                                                                                              ...
                                                                                              this.dangqianjinduOptions = "0%,40%,90%,100%".split(',')
                            },
                    // 下二随
    zhiweiChange () {
      this.$http({
        url: `follow/zhiweixinxi/zhiweiming?columnValue=`+ this.ruleForm.zhiwei,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
                                                                                                                                                      } else {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `gongzuojindu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.gongzuojinduCrossAddOrUpdateFlag = false;
                  this.parent.search();
